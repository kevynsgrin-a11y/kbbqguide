...
The interface now contains 115 active, local editorial assets: 80 finished-dish recipe heroes plus 35 distinct brand, category, collection, guide, menu, system, and policy frames. Every active asset is an original Phase 9 synthetic campaign image and is explicitly labeled `synthetic-labeled`; none is represented as documentary photography or a test-kitchen result. `data/media-manifest.json` is the machine authority and `docs/media-assets-phase9.csv` is the flat inventory.
...
Source masters are local 2400 × 1600 JPEGs. Astro produces AVIF and WebP sources plus JPEG fallback at 360, 640, 960, 1280, and 1600 widths. A build-safe `import.meta.glob` registry prevents missing dynamic asset paths. Detail, category, guide, menu, and home heroes are the only eager images and receive high fetch priority. Cards, banners, and policy textures are lazy. Each route has at most one eager image. Aspect-ratio reservations prevent layout shift, while validated focal-point and mobile-crop classes avoid CSP-incompatible inline styles.
